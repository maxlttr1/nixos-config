    ...
    home.shellAliases = {
      ...
      music-download = ''
        ${pkgs.yt-dlp}/bin/yt-dlp \
          "https://www.youtube.com/watch?v=AQ6iA-0CRL8&list=PLkF8ZEu4FB1kvxIhG9jLAEauSivb5JJGD" \
          -f "bestaudio" \
          --remux-video "opus" \
          --embed-metadata --embed-thumbnail \
          -o "$HOME/mountedDisk/syncthing/music/New/%(title)s [%(id)s].%(ext)s" \
          --download-archive "$HOME/mountedDisk/syncthing/music/downloaded.txt" \
          --cookies  "$HOME/mountedDisk/syncthing/music/cookies.firefox-private.txt"
      '';
      music-normalize = ''
        ${pkgs.rsgain}/bin/rsgain easy -p no_album --skip-existing "$HOME/mountedDisk/syncthing/music/"
      '';
    };
  };
}
